...
int global_iteration;//record the iteration
...
void print_actual_thread(int actual_thread){
	...
	printf("The actual number of OpenMP threads created: %d\n",actual_thread);
}
...
void* disspate_box(int tn){
	...
	if(index!=num_threads-1)//not last thread
	{
		...
		for(temp=index*each_iteration;temp<end_iteration;temp++)
		{
			boxes[temp].temperature=place_temp[temp];
		}
		#pragma omp barrier
		min_tem=min_temp();
		max_tem=max_temp();
...
void compute_tem(){
	...
	print_info(max_tem,min_tem,global_iteration,diff,clock_consum,seconds);
}
