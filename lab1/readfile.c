...
void compute_tem(){
	...
	int iteration=0;
	float*place_temp=malloc(sizeof(float)*num_box);
	int qq=0;
	for(qq=0;qq<num_box;qq++)//initial
		place_temp[qq]=0.0;
	while((max_tem-min_tem)>=max_tem*epsilon){
		int index=0;
	for(index=0;index<num_box;index++){
	float average_tem=cal_adjacent_tem(index);
	float new_tem;
	if(boxes[index].temperature<=average_tem)
	{
	new_tem=boxes[index].temperature+(average_tem-boxes[index].temperature)*affect_rate;
	}
	else
	{
		new_tem=boxes[index].temperature-(boxes[index].temperature-average_tem)*affect_rate;
	}
	place_temp[index]=new_tem;
	}//for iteration
	iteration++;
